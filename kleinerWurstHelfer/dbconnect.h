...
static bool DB_verbindung() 
{
	...
	QString dbPfadini = mySettings.getDatabasePath();

	if (dbPfadini == "") {
		dbPfadini = QDir::currentPath();
		//settings.setValue("DB_Pfad", dbPfadini);
		mySettings.setDatabasePath(dbPfadini);
	}
	//settings.endGroup();


	//dbPfad = QDir::homePath() + "/" + dbName;;
	//dbPfad = QDir::currentPath() + "/" + dbName;
	dbPfad = dbPfadini + "/" + dbName;
